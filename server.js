...
// Connection URL
var url = 'mongodb://localhost:27017/crud1';
// Use connect method to connect to the Server
MongoClient.connect(url, (err, db) => {
  if (err) return console.log(err);
  console.log("Connected correctly to server");

  var collection = db.collection('quotes3');

  app.get('/',(req,res) => {
    collection.find({}).toArray((err,docs) => {
         if (err) return console.log(err);
         console.log(docs);
         res.render('index1', { list : docs });
    })
});
  ...
  , {
    ...
    if (err) console.log(err);
    res.redirect("/")//post and then get to see the results, routing issue
  })
});
...
Next, we want to start our servers only when the database is connected.
Hence, let’s move app.listen into the connect method.

The $natural parameter returns items according to their natural order within the database.
This ordering is an internal implementation feature, and you should not rely on any particular
structure within it.
